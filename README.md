...
코드집 ★Jay\06.상품\▥상품코드집_운영.xlsx                  ← 정렬·유형·운영·제상품
제외  06_Stock-Supply\대시보드노출제외상품.xlsx           ← 화면에서 뺄 품목
      │
...
- **단종 제외**: 코드집 '운영'에 `단종`이 포함되고 **현재고 50개 이하**면 목록에서 뺀다. 단, **당월 판매가 있으면 유지**
  (기준값은 `build_data.py` 의 `EOL_STOCK_MAX = 50`)
- **노출 제외**: `대시보드노출제외상품.xlsx` 의 **'제품명' 열에 적힌 품목은 무조건 제외**. 빼고 싶은 품목이 생기면 이 파일에 행만 추가하면 된다 (모든 시트를 읽음)
- **제상품** 열: 코드집 `상품코드집` 시트 BG열(`상제품`) — 제품/상품
- **운영** 열: 코드집 운영 상태를 그대로 표시. 단종 계열은 빨간 글씨
- **회전일 필터**: 전체 / 품절 / 15일 미만 / 15~30 / 30~60 / 60일 이상 / 산출 불가
- **제상품 필터**: 전체 / 상품 / 제품
- **엑셀 내보내기**: 시트 2개 — ① 화면 표 그대로 ② `수급상세`(입고 팝업 내용을 품목별로 펼침). 화면 필터가 두 시트에 모두 적용됨
- **틀고정**: 판매뷰는 회전일까지, 재고뷰는 입고까지 고정 (가로 스크롤해도 남음)
